...
Noi is a macOS-first dictation app built on Handy: Tauri 2.x (Rust backend + React/TypeScript frontend), with a native Swift island overlay (`src-tauri/swift/`).
...
The app supports command-line parameters on all platforms for integration with scripts, window managers, and autostart configurations.
