...
fn build_apple_speech_bridge() {
    ...
    // SpeechAnalyzer arrived with the macOS 26 SDK; FoundationModels.framework
    // is a reliable marker for that SDK. Unlike the Apple Intelligence bridge
    // this needs no macro plugin, so a Command-Line-Tools swiftc is fine.
    let sdk_is_26 = Path::new(&sdk_path)
        .join("System/Library/Frameworks/FoundationModels.framework")
        .exists();
    let force_stub = env::var("HANDY_FORCE_SPEECH_STUB").as_deref() == Ok("1");
    let has_speech_analyzer = sdk_is_26 && !force_stub;

    let source_file = if has_speech_analyzer {
        println!("cargo:warning=Building with Apple Speech (SpeechAnalyzer) support.");
        REAL_SWIFT_FILE
    } else {
        println!("cargo:warning=Apple Speech: macOS 26 SDK not found. Building with stubs.");
        STUB_SWIFT_FILE
    };
