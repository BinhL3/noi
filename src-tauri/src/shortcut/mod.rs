...
#[tauri::command]
#[specta::specta]
pub fn change_notes_enabled_setting(app: AppHandle, enabled: bool) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
...
#[tauri::command]
#[specta::specta]
pub fn change_overlay_clock_setting(app: AppHandle, enabled: bool) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.overlay_clock = enabled;
    settings::write_settings(&app, settings);
    #[cfg(target_os = "macos")]
    crate::native_notch::set_clock(enabled);
    Ok(())
}
